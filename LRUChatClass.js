...
class LRUChatClass{
    ...
    createNode(id,UserName,msgText){
        ...
        chat_item.querySelector('#Name').innerText = UserName;
        chat_item.querySelector('#Message').innerText = msgText;
    ...
    newMsg(id,UserName,msgText){
        let node = null;
        if((id in this.hashmap ) === false){
            // If node not in linked list
            node = this.createNode(id,UserName,msgText);
